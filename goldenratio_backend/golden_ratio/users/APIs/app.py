...
# Process the image with MediaPipe face mesh
    ...
    for dist1, dist2, desc1, desc2, ref1_val, ref2_val in zip(inpt_arr, inpt1_arr, Name, Name2, rft_arr, rft1_arr):
        p = dist1 * 1.618
        q = p / dist2
        ...
        if q * 100 > 100:
            ...
            # q * 100
            ratios1.append({
                'Description1': f"{desc1}",
                'Description2': f"{desc2}",
                'dist1': f"{dist1:.3f}",            # patient value 1
                'dist2': f"{dist2:.3f}",            # patient value 2
                'ref1': f"{ref1_val:.3f}",          # reference value 1
                'ref2': f"{ref2_val:.3f}",          # reference value 2
